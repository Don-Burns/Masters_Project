...
\section{Introduction}
	\linenumbers
%	TODO how my model is generic to any taxon
...
%	that is movement, digestion et cetera.  
	...
	The framework used by \citeauthor{West2001} (\citeyear{West2001}) was latter developed by \citeauthor{Charnov2001} (\citeyear{Charnov2001}) to take the cost of reproduction into account and allowing the estimation of lifetime production of offspring.  This was done by adding an additional cost which dedicated some portion of mass to reproductive output, essentially gonadosomatic index (GSI).	%like 20% max? in fish very high in  invertabrates \cite{Parker2018}
...
% 10 \% predicted under Roff 1983
	...
	As can be seen in Figure \ref{growth_curve}, growth is very fast within the model.  Asymptotic mass is reached by $\sim$10 days.  This makes interpreting any results regarding maturation time difficult % TODO < rephrase
	because any time after $\sim$10 days produces the same result (see Fig. \ref{fig:sensmaturationtimelong}, \ref{fig:sensmaturationtimeshort} and \ref{fig:senslowresourcesmaturationtimeshort}).
	
	\subsection{Resource Density}
	% talking about at meta_exp = 1
	Increasing resource density leads to an increase in $\rho$.  Hyperallometric scaling was observed in both dimensions (see Fig. \ref{fig:senslowresourcesresourcedensity}, \ref{fig:sensresourcedensity}).  However, in 3D scaling stays between 1.2 and 1, with occasional dips below one. Perhaps due to numeric instability within the model (Fig. \ref{fig:senslowresourcesresourcedensity}).  Once resource density exceeds ?? kg/m$^3$ in 3D resources are saturated and no change will occur even if density is increased (Fig. \ref{fig:senslowresourcesresourcedensity})
	In 2D resources become saturated at ?? kg/m$^2$. % check dimensionality paper for means of comparing the two values and if there is none point out that there is a difference due to dimensions
	
	\subsection{Metabolic Exponent}
	% talking about at meta_exp = 1
	When metabolic exponent is increased, so too does $\rho$.  This is contrary to what would be expected given that a lower exponent should allow for steeper metabolic scaling (see \citeauthor{Marshall2019} (\citeyear{Marshall2019})).  % Need an explanation for this.
	
	
	% TODO need to produce results with exponent at 0.75 and discuss those.
...
\section{Discussion}
	...
	Some caveats with the results of the model.  First, growth is simulated as being extremely fast.  As previously stated, asymptotic size was reach by $\sim$10 days.  This is of course not representative of the real world, where individuals generally need several months to years to reach maturity.  (need source)
	
	This may be due to several factors.  First is that metabolic cost may be underestimated.  Similar to \citeauthor{West2001} (\citeyear{West2001}), this study used resting metabolic rate to define metabolic costs.  However, this does not take other costs into account such as digestion and locomotion.
	...
	In this regard, temperature plays a critical role.  It is well documented that a change in temperature will change many biological rates.  For homeotherms this can come as a result of environmental temperature change % source
	and/or as a result of lowered metabolic rate, for example in times of starvation. %sources \cite{Hou2008} has a lead.
	In poikilotherms ambient temperature also plays a role.  It has been shown that growth is dependent on temperature within fish. % sources
	Fish sizes are expected to decrease due to climate change by as much as XX\%. %source
	The functional response data used in this study is standardised around 15\textdegree{}C.
	Meanwhile the metabolic cost is for an unspecified temperature.  Using rates where the temperature effect is taken into account.  Work such as \citeauthor{Barneche2014} (\citeyear{Barneche2014}) has investigated this effect, however the estimate for metabolic rate is several orders of magnitude lower than what was reported by \citeauthor{Peters1983} (\citeyear{Peters1983}), which is the rate used in this study.  Thus further investigation is needed.  % should include a plot in the SI for this.
